...
function getNextRoom(arr, position){
  ...
  switch (position) {
    case "top":
      return [arr[0], arr[0]+sizex, arr[2]-wallsize-sizey, arr[2]-wallsize]
    case "right":
      return [arr[1]+wallsize, arr[1]+wallsize+sizex, arr[2], arr[2]+sizey]
    case "bottom":
      return [arr[1]-sizex, arr[1], arr[3]+wallsize, arr[3]+wallsize+sizey]
    case "left":
      return [arr[0]-wallsize-sizex, arr[0]-wallsize, arr[3]-sizey, arr[3]]
    default:
      return undefined
  }
}
...
//    }
    ...
    if(((room[1]<newroom[0])||(room[0]>newroom[1])) && ((room[3]<newroom[2])||(room[2]>newroom[3]))){
...
function translateIntoArray() {
  ...
  for (var i=0; i<rooms.length; i++) {
    for (var y=rooms[i][2]; y<rooms[i][3]; y++) {
      for (var x=rooms[i][0]; x<rooms[i][1]; x++) {
        mapArray[y][x]= 0;
      }
    }
  }
  ...
  for (var i=0; i<doors.length; i++) {
    for (var y=doors[i][2]; y<doors[i][3]; y++) {
      for (var x=doors[i][0]; x<doors[i][1]; x++) {
        mapArray[y][x]= 0;
      }
    }
  }
...
function placeItems() {
  //player
  var choice = choosePlace(itemsArray)
  itemsArray[choice[1]][choice[0]] = 4

  for (var z=0; z< heathItems; z++) {
    var choice = choosePlace(itemsArray)
    itemsArray[choice[1]][choice[0]] = 1
  }
  for (var z=0; z< weaponItems; z++) {
    var choice = choosePlace(itemsArray)
...
function choosePlace(array) {
  ...
  if ( array[ry][rx] === 0 ) { return [rx, ry] }
  else {choosePlace(array)}
}
